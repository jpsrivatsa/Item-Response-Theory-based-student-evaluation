...
def func(theta, response_vector: List[bool], items: numpy.ndarray):
    return log_likelihood(theta,response_vector,items)
    
 
# Derivative of the above function
# which is 3*x^x - 2*x
def derivFunc(theta, response_vector: List[bool], items: numpy.ndarray ):
    derval = 0
    a, b, c, d = _split_params(items)
    for i in range (0,len(response_vector)):
        psi = icc(theta,a[i],b[i],c[i],d[i])
        
        derval += a[i] * exp(-a[i] * (theta-b[i])) * (response_vector[i]/psi - (1-response_vector[i])/(1-psi)) / ((psi-c[i])*(psi-c[i]))
    return derval
 
# Function to find the root
def newtonRaphson( theta, response_vector: List[bool], items: numpy.ndarray):
    
    for i in range(0,100):
        h = func(theta, response_vector, items) / derivFunc(theta, response_vector, items)
        # x(i+1) = x(i) - f(x) / f'(x)
        theta = theta - h
    print("The value of the root is : ","%.4f"% theta)
...
def validate_item_bank(items: numpy.ndarray, raise_err: bool = False):
    ...
    print(err)
def estimate_ability(responses, difficulty):
    """
    Estimates the ability of a person given a set of responses
    and a difficulty rating. 

    Parameters
    ----------
    responses : list
        A list of responses given by the person 
    difficulty : list
        A vector of difficulty levels corresponding to the responses

    Returns
    -------
    ability : float
        The estimated ability of the person

    """

    responses_len = len(responses)
    difficulty_len = len(difficulty)
    if responses_len != difficulty_len:
        raise ValueError("Length of responses and difficulty vectors must be equal")

    b = np.average(responses) - np.average(difficulty)
    a = np.average(responses * difficulty) - np.average(responses) * np.average(difficulty)

    if a < 0:
        raise ValueError('Cannot estimate ability with negative item discrimination')

    ability = a * b * (responses_len + 1) / (a * responses_len + b)

    return ability
rv = list([0,1,1,1,1,1,1,0])
qp = [[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1],[1,4.0,0.25,1]]
theta = 0 # Initial values assumed
newtonRaphson(theta,rv,numpy.array(qp))
